...
class ProtocolBase(asyncio.Protocol):
    ...
    def data_received(self, data):
        ...
        self.ef_sender = asyncio.ensure_future(self.sender.put(data))
...
def create_tcp_visonic_connection(
    ...
    protocol = partial(protocol, receiver=receiver, sender=sender, name=name, deb=debs, loop=loop or asyncio.get_event_loop(),)
    return loop.create_connection(protocol, address, port)
...
def create_usb_visonic_connection(
    ...
    return create_serial_connection(loop, protocol, port, baud)
...
if connpc is not None and connalarm is not None:
    _a = testloop.create_task(connpc)
    _b = testloop.create_task(connalarm)
