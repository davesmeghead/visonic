...
def create_default(options, key, default):
 ...
 #           if key == CONF_MOTION_OFF_DELAY:
 #               _LOGGER.info("create_default CONF_MOTION_OFF_DELAY default = {0}".format(options[key]))
            return options[key]
    else:
        return default
...
def create_schema_device():
    #_LOGGER.info("create_schema_device A {0}".format(CONFIG_SCHEMA_DEVICE))
    #fred = vol.Schema(CONFIG_SCHEMA_DEVICE)
    #_LOGGER.info("create_schema_device B {0}".format(fred))
...
initialise = {**CONFIG_SCHEMA_DEVICE, **CONFIG_SCHEMA_ETHERNET, **CONFIG_SCHEMA_USB, **create_parameters1(options), **create_parameters2(options), **create_parameters3(options), **create_parameters4(options)}

for key in initialise:
    d = key.default()
    options[key] = d
    #_LOGGER.info("initialising options dictionary {0} {1}".format(key, options[key]))
